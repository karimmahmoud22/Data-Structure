...
private:
	...
	static bool rec_search(Node* subRoot , int value);
	static int rec_less(Node* subRoot, int value);
	static bool rec_PrintSmallerEq(Node* subRoot, int& key);
...
public:
	...
	int less(int value) const;
	bool PrintSmallerEq(int & key) const;
